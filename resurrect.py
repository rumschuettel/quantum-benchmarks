...
sys.modules["qiskit.circuit.bit"].__dict__["Bit"].__hash__ = lambda self: hash(
...
sys.modules["qiskit.circuit.register"].__dict__["Register"].__hash__ = lambda self: hash(
    (type(self), self.name, self.size)
)
...
def iterate_obj(o):
    if isinstance(o, dict):
        ...
        return set(iterate_obj(e) for e in o)
    elif isinstance(o, TYPES_TO_SKIP):
        return o
    elif o is None:
        return o
    elif type(o).__name__ == "QuantumCircuit":
        return "Was saved in an obsolete format, so it is deleted."

    # Iterate over the children
    for x in dir(o):
        if x.startswith("__"):
            continue
        if isinstance(getattr(o, x), TYPES_TO_SKIP):
            continue
        if type(getattr(o, x)).__name__ in [
            "method",
