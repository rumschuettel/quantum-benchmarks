...
class HHLBenchmarkMixin:
    ...
    def __init__(self, block_encoding, num_qubits, num_ancillas, qsvt_poly, num_shots, shots_multiplier,**_):
        super().__init__()
...
def argparser(toadd, **argparse_options):
    parser = toadd.add_parser(
        "HHL", help="HHL QSVT benchmark.", **argparse_options
    )
    parser.add_argument(
        "-b", "--block_encoding", type=object, help="The block-encoding of the matrix A", default=None
    )
    parser.add_argument(
        "-q", "--num_qubits", type=int, help="The number of qubits used by A", default=2
    )
    parser.add_argument(
        "-a", "--num_ancillas", type=int, help="The number of ancillas used by the block-encoding", default=1
    )
    parser.add_argument(
