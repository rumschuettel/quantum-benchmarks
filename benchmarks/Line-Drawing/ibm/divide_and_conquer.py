...
def divide_and_conquer(circuit, qubits, ancilla_qubits, points, ctrl = None):
    ...
    if abs(theta) > 1e-4:
        if ctrl is not None: circuit.cu3(theta * np.pi, 0, 0, ctrl, qubits[0])
        else: circuit.ry(theta * np.pi, qubits[0])
        # if ctrl is not None: circuit.rz(-.5 * np.pi * theta, ctrl)

    if abs(phi) > 1e-4:
        if ctrl is not None: circuit.crz(phi * np.pi, ctrl, qubits[0])
        else: circuit.rz(phi * np.pi, qubits[0])
        if ctrl is not None: circuit.rz(.5 * phi * np.pi, ctrl)
    ...
    if n > 1:
        ...
        if np.linalg.norm(one_part) < 1e-4:
            ...
            divide_and_conquer(circuit, new_qubits, new_ancilla_qubits, points[2**(n-1):], new_ctrl)
            if ctrl is not None: circuit.cx(ctrl, new_ctrl)
            else: circuit.x(new_ctrl)
            divide_and_conquer(circuit, new_qubits, new_ancilla_qubits, points[:2**(n-1)], new_ctrl)
            if ctrl is not None:
