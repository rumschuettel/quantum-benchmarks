...
def Shende_Bullock_Markov(circuit, qubits, state, GRAY_CODE = True, REVERSE_ZS = True):
    ...
    for i in range(n):
        ...
        prepare_multiplexed(circuit, qubits[:i], qubits[i], states, GRAY_CODE = GRAY_CODE, REVERSE_ZS = REVERSE_ZS)
    return circuit

def prepare_multiplexed(circuit, control_qubits, target_qubit, states, GRAY_CODE = True, REVERSE_ZS = True):
